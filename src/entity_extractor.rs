...
impl SplatsDBEntityExtractor {
    ...
    pub fn with_config(config: ExtractorConfig) -> Self {
        Self {
            email_re: Regex::new(r"(?i)[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}")
            ...
            stopwords: [
                ...
                "con", "en", "para", "por", "y", "o", "u", "e", "que", "es", "son", "se", "su",
                "lo", "the", "a", "an", "and", "or", "in", "on", "at", "to", "for", "with", "is",
                "are", "of", "from", "by", "this", "that", "it", "as", "was", "were", "be", "been",
                "has", "have",
            ]
            .iter()
            .map(|s| s.to_string())
    ...
    fn extract_ngrams(&self, text: &str, candidates: &mut HashMap<String, EntityCandidate>) {
        let tokens: Vec<(&str, usize)> = self
            .word_re
            .find_iter(text)
            .map(|m| (m.as_str(), m.start()))
            .collect();

        let connectors: HashSet<&str> = ["de", "del", "la", "el", "y", "e", "u"]
            .iter()
            .copied()
            .collect();
        let mut i = 0;
        ...
        while i < tokens.len() {
            ...
            if is_capitalized(word) && !self.stopwords.contains(&word.to_lowercase()) {
                ...
                while j < tokens.len() && j - i < 4 {
                    ...
                    if is_capitalized(next_word)
                        && !self.stopwords.contains(&next_word.to_lowercase())
                    {
...
mod tests {
    ...
    fn test_extract_date() {
        ...
        assert!(results
            .iter()
            .any(|c| c.entity_type == "date" && c.text == "2026-03-28"));
    }
    ...
    fn test_extract_email() {
        ...
        assert!(results
            .iter()
            .any(|c| c.entity_type == "contact" && c.text.contains("@")));
    }
    ...
    fn test_extract_proper_nouns() {
        ...
        let proper: Vec<_> = results
            .iter()
            .filter(|c| c.entity_type == "proper_noun")
            .collect();
