...
pub fn cmd_status(data_dir: String, max_splats: usize, config: SplatDBConfig, verbose: bool) {
    ...
    println!(
        "{}",
        serde_json::to_string_pretty(&out).expect("Failed to serialize status JSON")
    );
}
...
pub fn cmd_soc_check(data_dir: String, config: SplatDBConfig) {
    ...
    soc.update_splats(
        ...
        kappa.expect("kappa checked above"),
    );
...
pub fn cmd_save(data_dir: String, config: SplatDBConfig, shard: String) {
    ...
    let persist = match persist {
        Some(p) => p,
        None => {
            eprintln!("Error: no persistence layer");
            std::process::exit(1);
        }
    };
    ...
    if let (Some(mu), Some(alpha), Some(kappa)) = (mu, alpha, kappa) {
        ...
        persist.save_vectors(&vectors, &shard).unwrap_or_else(|e| {
            eprintln!("Save error: {}", e);
            std::process::exit(1);
        });
        persist
            .save_energy_state(&vectors, &alpha.to_vec(), &kappa.to_vec())
            .unwrap_or_else(|e| {
                eprintln!("Energy save error: {}", e);
                std::process::exit(1);
            });
        println!(
            "{}",
...
pub fn cmd_load(data_dir: String, backend: String, shard: String) {
    let persist = make_persistence(&data_dir, &backend, true).unwrap_or_else(|e| {
    ...
    match persist.load_vectors(&shard) {
        ...
        Ok(None) => {
            println!(
                "{}",
                serde_json::json!({"status": "not_found", "shard": shard})
            );
        }
        Err(e) => {
...
pub fn cmd_list(data_dir: String, backend: String) {
    let persist = make_persistence(&data_dir, &backend, false).unwrap_or_else(|e| {
        eprintln!("Storage error: {}", e);
        std::process::exit(1);
    });
...
pub fn cmd_doc_add(
    ...
    persist
        .save_metadata(&id, 0, 0, meta_json.as_ref(), Some(&text))
        .unwrap_or_else(|e| {
            eprintln!("Error: {}", e);
            std::process::exit(1);
        });
...
pub fn cmd_doc_get(data_dir: String, backend: String, id: String) {
    ...
    match persist.get_metadata(&id) {
        Ok(Some(meta)) => println!(
            "{}",
            serde_json::to_string_pretty(&meta).expect("Failed to serialize metadata")
        ),
...
pub fn cmd_backup(data_dir: String, backend: String, output: PathBuf) {
    ...
    match persist.backup(output.to_str().unwrap_or("./backup")) {
        Ok(path) => println!(
            "{}",
            serde_json::json!({"status": "backup_ok", "path": path})
        ),
        Err(e) => {
...
pub fn cmd_preset_info(preset: Option<String>) {
    ...
    let info: Vec<serde_json::Value> = filtered
        .iter()
        .map(|(name, c)| {
    ...
    println!(
        "{}",
        serde_json::to_string_pretty(&info).expect("Failed to serialize preset info")
    );
}
