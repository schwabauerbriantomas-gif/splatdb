...
impl<E: EnergyFn> EbmExploration<E> {
    ...
    pub fn find_high_energy_regions(
        ...
        let mut high_energy: Vec<(Vec<f32>, f64)> = candidates
            .into_iter()
            .filter_map(|pt| {
    ...
    pub fn suggest_exploration(&mut self, n_suggestions: usize) -> Vec<ExplorationSuggestion> {
        if self.all_vectors.is_empty() {
            return Vec::new();
        }
        ...
        for region in regions.into_iter().take(n_suggestions) {
            // Find nearby vectors
            let mut nearby: Vec<(usize, f64)> = self
                .all_vectors
                .iter()
                .enumerate()
                .map(|(i, v)| {
            ...
            let nearby_ids: Vec<String> = nearby
                .iter()
                .take(5)
                .map(|(i, _)| {
                    self.all_ids
            ...
            suggestions.push(ExplorationSuggestion {
                suggested_queries: vec![
                    ...
                    format!(
                        "Search complementary info in energy region {:.2}",
                        region.energy
                    ),
                ],
                description: desc,
    ...
    fn next_vec(&mut self, dim: usize) -> Vec<f32> {
        (0..dim)
            .map(|_| (self.next_random() * 2.0 - 1.0) as f32)
            .collect()
    }
}
...
mod tests {
    ...
    fn test_suggest_exploration() {
        ...
        exp.update_vectors(vectors, (0..10).map(|i| format!("v{}", i)).collect());
        let _suggestions = exp.suggest_exploration(2);
    }
