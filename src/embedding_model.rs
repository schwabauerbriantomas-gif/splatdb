...
pub fn matryoshka_all(embedding: &[f32], dims: &[usize]) -> Vec<Vec<f32>> {
    dims.iter()
        .map(|&d| matryoshka_truncate(embedding, d))
        .collect()
}
...
impl ProjectionHead {
    ...
    pub fn new(input_dim: usize, output_dim: usize, seed: u64) -> Self {
        ...
        let weight: Vec<Vec<f32>> = (0..output_dim)
            .map(|_| {
                (0..input_dim)
                    .map(|_| {
    ...
    pub fn forward_matryoshka(&self, input: &[f32], dims: &[usize]) -> Vec<Vec<f32>> {
        ...
        dims.iter()
            .map(|&d| matryoshka_truncate(&full, d))
            .collect()
    }
...
pub fn compute_distillation_loss(
    ...
    let mse: f32 = student[..min_dim]
        .iter()
        .zip(teacher[..min_dim].iter())
        .map(|(&s, &t)| {
            let d = s - t;
            d * d
        })
        .sum::<f32>()
        / min_dim.max(1) as f32;
...
mod tests {
    ...
    fn test_distillation_loss() {
        ...
        assert!(
            loss.cosine < 0.01,
            "Identical vectors should have near-zero cosine loss"
        );
        assert!(
            loss.mse < 0.01,
            "Identical vectors should have near-zero MSE"
        );
    }
    ...
    fn test_distillation_loss_different() {
        ...
        assert!(
            loss.cosine > 0.5,
            "Orthogonal vectors should have high cosine loss"
        );
